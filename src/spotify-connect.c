...
int json_parse_playback_info(const char *json, int n, char *output, size_t output_size) {
...
int playback_info(CURL *curl) {
    ...
    char url[64];
    snprintf(url, sizeof(url), "%s/currently-playing", SPOTIFY_API_BASE);
    ...
    if (res == CURLE_OK) {
        ...
        json_parse_playback_info(response_buffer, ALBUM, album, sizeof(album));
        json_parse_playback_info(response_buffer, ARTIST, artist, sizeof(artist));
        json_parse_playback_info(response_buffer, TRACK, track, sizeof(track));
...
int main(int argc, char *argv[]) {
    if (argc < 2) {
        ...
        printf("  prev    - Skip to previous track\n");
